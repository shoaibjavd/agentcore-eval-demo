...
This is the recommended approach for most CI pipelines. It decouples evaluation
from live MCP calls entirely — you collect traces during manual testing or staging
runs, commit them as JSON fixtures, and evaluate those in CI. Same traces = same
scores = deterministic quality gate.
"""
import boto3
import json
import os
...
# Reuse the evaluator definitions from evaluate_agent.py
...
def load_trace_fixtures(fixtures_dir):
    ...
    Returns:
        list: Combined list of OTel spans from all fixture files.
    """
    all_spans = []
...
def evaluate_and_gate(spans, evaluator_ids, threshold, region):
    ...
    Args:
        spans: List of OTel spans (from fixtures).
        evaluator_ids: List of evaluator IDs to run.
        threshold: Minimum score to pass the quality gate.
        region: AWS region for the Evaluate API.

    Returns:
        bool: True if all evaluators meet the threshold.
    """
    # Use the data plane client for evaluation
...
def main():
    ...
    fixtures_dir = os.environ.get("TRACE_FIXTURES_DIR", "tests/fixtures/traces")
